...
const fetchJSON = async (url, options = {}, timeoutMs = 5000) => {
  ...
  try {
    ...
    if (!data || data.success === false) throw new Error((data && data.error) || 'Permintaan gagal');
    return data;
  } finally {
    clearTimeout(t);
  }
...
const normalizeState = (data) => ({
  ...
  code: data.code && data.code.salt && data.code.hash ? { salt: data.code.salt, iterations: data.code.iterations, hash: data.code.hash } : null,
  qa: data.qa && data.qa.salt && data.qa.hash ? { question: data.qa.question || null, salt: data.qa.salt, iterations: data.qa.iterations, hash: data.qa.hash } : null,
  updatedAt: data.updatedAt || null,
});

// Muat konfigurasi kunci dari database; cache ke IndexedDB untuk offline.
const loadServerState = async () => {
  const data = await fetchJSON('/api/auth/state', { method: 'GET' });
  const state = normalizeState(data);
  await kvSet(KEYS.dbConfig, state);
  return state;
};

const ensureConfig = async () => {
  const cached = await kvGet(KEYS.dbConfig);
  if (cached) return cached;
  return loadServerState().catch(() => null);
};
...
export async function hashSecret(salt, secret, iterations = PBKDF2_ITERATIONS) {
...
// ── Setup state (bersumber dari database, fallback cache offline) ───
export async function getSetupState() {
  ...
  try {
    ...
    online = false;
    state = await kvGet(KEYS.dbConfig);
  }

  const s = state || { done: false, hasCode: false, hasQA: false, question: null };
  const passkey = isWebAuthnSupported() ? await kvGet(KEYS.passkey) : null;
...
export async function getQuestion() {
  ...
  return cfg && cfg.qa ? cfg.qa.question : null;
}
...
// ── Security question (tersimpan di database) ───────────────────────
export async function setQuestion(question, answer) {
  const iterations = PBKDF2_ITERATIONS;
  const salt = makeSalt();
  const hash = await hashSecret(salt, (answer || '').trim().toLowerCase(), iterations);
  const qa = { question: (question || '').trim(), salt, iterations, hash };
  ...
  try {
    ...
    await kvSet(KEYS.dbConfig, normalizeState(data));
  } catch (err) {
    // Simpan lokal sebagai cadangan agar perangkat yang sudah pernah online
    // tetap bisa memverifikasi secara offline.
    const prev = (await kvGet(KEYS.dbConfig)) || { done: false, hasCode: false, hasQA: false, question: null, code: null };
    await kvSet(KEYS.dbConfig, {
      ...prev,
      done: true,
      hasQA: true,
...
export async function verifyAnswer(answer) {
  const cfg = await ensureConfig();
  if (!cfg || !cfg.qa) return false;
  const hash = await hashSecret(cfg.qa.salt, (answer || '').trim().toLowerCase(), cfg.qa.iterations);
  return hash === cfg.qa.hash;
}

// ── Access code (tersimpan di database) ─────────────────────────────
export async function setCode(code) {
  const iterations = PBKDF2_ITERATIONS;
  const salt = makeSalt();
  const hash = await hashSecret(salt, String(code).trim(), iterations);
  const rec = { salt, iterations, hash };

  try {
    const data = await fetchJSON('/api/auth/setup', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ code: rec }),
    });
    await kvSet(KEYS.dbConfig, normalizeState(data));
  } catch (err) {
    const prev = (await kvGet(KEYS.dbConfig)) || { done: false, hasCode: false, hasQA: false, question: null, qa: null };
    await kvSet(KEYS.dbConfig, {
      ...prev,
      done: true,
      hasCode: true,
...
export async function verifyCode(code) {
  const cfg = await ensureConfig();
  if (!cfg || !cfg.code) return false;
  const hash = await hashSecret(cfg.code.salt, String(code).trim(), cfg.code.iterations);
  return hash === cfg.code.hash;
}
